...
**Restored for launch (S7-02):** `settings/audit-log` (S-24) and `sync/[runId]`
(S-26) were re-enabled. Audit visibility is a stated security baseline item, and
the `reconcile-xero-approval-state` job emits live `actionUrl: /sync/<runId>`
notification deep links, so the run detail screen is a linked route rather than a
deferred one. Both screens, their navigation/links, and the recent audit events
dashboard card are now in the build path.
